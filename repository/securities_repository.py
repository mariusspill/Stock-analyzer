...
def add_entry_securities(id: int, ticker: str, isin: str, wkn: str):
    ...
    """
    sql = """INSERT INTO securities (company_id, ticker, isin, wkn) VALUES (%s, %s, %s, %s);"""
...
def get_id_by_ticker(ticker: str):
    """
    Looks up the securities table to translate a public 
    string ticker into your internal relational integer company_id.
    """
    sql = "SELECT company_id FROM securities WHERE ticker = %s;"
...
def get_securities(company_id: int):
    db.connection.commit()

    sql = """
            SELECT * FROM securities WHERE company_id = %s;
        """
